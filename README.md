...
### 智能路由
- **Provider 注册表** — KV 驱动，动态添加模型供应方无需改代码，支持自定义请求头（解决 WAF 拦截）
- **多账号 Key 池** — 多个 API Key 自动轮询/优先级 failover，指数退避，按模型级锁定
- **模型路由按优先级** — 同一模型名在多个 provider 中存在时，按 priority 数字排序选择（数字越小越优先）
...
| 模型 | Provider | 输入价格 | 输出价格 | 说明 |
|------|----------|----------|----------|------|
| glm-latest | 火山引擎 ARK | $0.14/M | $0.28/M | GLM 最新版 |
| doubao-seed-2.0-pro | 火山引擎 ARK | $0.11/M | $0.43/M | 豆包旗舰版 |
| ark-code-latest | 火山引擎 ARK | $0.11/M | $0.43/M | ARK 代码版 |
| doubao-1-5-thinking-pro-32k | 火山引擎 ARK | $0.11/M | $0.43/M | 豆包思考版 |
| kimi-for-coding | Kimi Code | $0.14/M | $0.42/M | Kimi 编程版 |
| kimi-for-coding-highspeed | Kimi Code | $0.07/M | $0.21/M | Kimi 编程高速版 |
| deepseek-chat | DeepSeek | $0.14/M | $0.28/M | DeepSeek V3 |
| deepseek-reasoner | DeepSeek | $0.55/M | $2.19/M | DeepSeek R1 思考版 |
| deepseek-v4-pro | DeepSeek | $0.55/M | $2.19/M | DeepSeek V4 Pro |
| mimo-v2.5 | 小米 MiMo | $0.10/M | $0.30/M | MiMo 轻量版 |
| mimo-v2.5-pro | 小米 MiMo | $0.10/M | $0.30/M | MiMo Pro 版 |

可通过管理后台 **Providers tab** 动态添加更多 Provider 和模型，无需改代码重新部署。
...
支持 `headers` 字段自定义请求头，用于绕过上游服务的 WAF 拦截：
...
```json
{
  ...
  "models": { "kimi-for-coding": {"input": 0.14, "output": 0.42} }
}
```

同一模型在多个 Provider 中存在时，按 `priority` 数字升序选择（数字越小越优先）。
...
### 内置 Combo 组合（可通过 Admin 管理）

| Combo 名 | 策略 | 模型列表 | 用途 |
|----------|------|---------|------|
| `fast-cheap` | round-robin | deepseek-chat → doubao-seed-2.0-pro → mimo-v2.5 | 通用轻量轮询 |
| `glm-pool` | round-robin | glm-latest × 2（双 key 轮询） | GLM 高并发 |
| `coding-pool` | fallback | ark-code-latest → kimi-for-coding | 编程专用 |
| `fast-all` | round-robin | doubao + glm + kimi + ark-code | 最大吞吐 |
| `deepseek-pool` | fallback | deepseek-chat → deepseek-reasoner | DeepSeek 兜底 |
| `default` | fallback | doubao→deepseek→glm→kimi→mimo | 万能兜底 |

所有工具统一配置：**Base URL = `https://api.silkgateway.ai/v1`**，**model = combo 名**，系统自动路由。
